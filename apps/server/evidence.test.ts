...
const historyOf = (tenant: Tenant, id: string) =>
  withOrganization(db, tenant.organizationId, (tx) =>
    tx
      ...
      // In the order it happened; without this the order is the plan's.
      .orderBy(asc(schema.auditEvent.createdAt), asc(schema.auditEvent.id)),
  );
...
describe("amending evidence", () => {
  ...
  it("writes nothing, and keeps its version, when nothing changes", async () => {
    // An UPDATE would move the version anyway, staling the tag an attester is
    // about to quote while history said nothing happened. The same instant in
    // another offset is the same value.
    const evidence = await record(acme, control, {
      title: "Unchanged",
      occurredAt: "2026-07-01T09:00:00.000Z",
    });
    const read = (await request(acme, `/evidence/${evidence.id}`)).headers.get("etag");

    const response = await amend(acme, evidence.id, {
      title: "Unchanged",
      occurredAt: "2026-07-01T11:00:00.000+02:00",
    });
...
describe("attesting", () => {
  ...
  it.each([
    ["a wildcard", () => "*"],
    ["a list holding the right tag", (tag: string) => `${tag}, "other"`],
    ["the right tag made weak", (tag: string) => `W/${tag}`],
    ["an empty value", () => ""],
  ])("refuses to sign with %s instead of the exact strong tag", async (_case, header) => {
    // Everywhere else If-Match asks "has this moved?"; a signature is of one
    // version in particular, so only that tag, exactly, will do (ADR 0012).
    const evidence = await record(acme, control, { title: "Signed exactly" });
    const tag = (await request(acme, `/evidence/${evidence.id}`)).headers.get("etag")!;

    const response = await attest(acme, evidence.id, header(tag));

    expect(response.status).toBe(412);
    const after = await json<{ data: Evidence }>(await request(acme, `/evidence/${evidence.id}`));
    expect(after.data.attestation).toBeNull();
  });

  it("attests what was just recorded, with the tag it came back with", async () => {
    const recorded = await request(acme, `/controls/${control}/evidence`, {
      method: "POST",
      body: JSON.stringify({ title: "Signed at once", occurredAt: "2026-07-01T09:00:00.000Z" }),
    });
    const { data } = await json<{ data: Evidence }>(recorded);
    const tag = recorded.headers.get("etag");
    expect(tag).toMatch(/^".+"$/);

    // Sent directly: the `attest` helper would read the tag itself if absent.
    const response = await request(acme, `/evidence/${data.id}/attestation`, {
      method: "PUT",
      headers: { "if-match": tag! },
    });
...
describe("dates this API will not take", () => {
  it.each([
    ["four minutes ahead, inside the allowance", 4, 201],
    ["six minutes ahead, past it", 6, 400],
  ])("treats a date %s as published", async (_case, minutes, status) => {
    // Five minutes absorb a client clock running ahead; a minute either side
    // keeps the test clear of the boundary.
    const response = await request(acme, `/controls/${control}/evidence`, {
      method: "POST",
      body: JSON.stringify({
        title: `A little ahead: ${minutes}`,
        occurredAt: new Date(Date.now() + minutes * 60 * 1000).toISOString(),
      }),
    });
  ...
  it.each([
    ...
    // Each parses as a JavaScript date. The positive offset moves year 0001
    // back into year zero; the negative offset moves year 9999 into the
    // expanded +010000 form. Neither fits the API's UTC year range, and
    // without that bound each reaches PostgreSQL as a 500 rather than being
...
describe("discarding evidence", () => {
  it("removes an unattested record", async () => {
