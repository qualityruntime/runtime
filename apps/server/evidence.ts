...
const notInTheFuture = () =>
  instant()
    .refine((value) => Date.parse(value) <= Date.now() + clockSkew, "Must not be in the future.")
    .meta({ description: "When the thing happened. Must not be in the future." });
...
/** Only a draft can be amended, so every field here is optional. */
const amendBody = z
  .object({
...
const staleEvidence = (c: Context) =>
  c.json(
    failure("precondition_failed", "The evidence changed since it was read.", [
 ...
 *
 * Takes the transaction it runs in, which should be repeatable read: the page
 * and its attachments are separate statements, and a file landing between them
 * would appear against evidence the page read before it existed.
 */
...
export const evidence = new Hono<OrganizationEnv>()
  ...
  .patch("/evidence/:evidenceId", knownEvidenceId, jsonBody(amendBody), async (c) => {
    ...
    const result = await c.var.withOrganization(async (tx) => {
      ...
      // Now that it is known to be a draft, lock it. Without this two
      // amendments can read the same values, and the second records no change
      // because what it wrote matches what it read rather than what was there.
      const [locked] = await tx
        .select({ ...getTableColumns(schema.evidence), version })
        .from(schema.evidence)
        .where(eq(schema.evidence.id, evidenceId))
        .for("update");
      // Attested between the read and the lock: an attested row cannot be
      // locked, because locking one means being able to update it.
      ...
      // Attested between the read and the write: the policy is the authority,
      // and it has just said no.
      if (!row) return { outcome: "attested" } as const;

      const changed = diffFields(fieldsOf(locked, audited), fieldsOf(row, audited));
      if (changed) {
        await c.var.audit(tx, {
          action: "updated",
          resourceType: "evidence",
  ...
  .delete("/evidence/:evidenceId", knownEvidenceId, async (c) => {
    ...
    // Unattested evidence only. Attesting is a signature, and what was signed
    // is kept — the DELETE policy has seen only unattested rows since evidence
    // arrived (ADR 0012); what was missing was a way to ask. Without it a draft
    // control that ever had evidence recorded against it could not be
    // discarded either, which left ADR 0017 advising something impossible.
    const result = await c.var.withOrganization(async (tx) => {
      ...
      if (!current) return { outcome: "missing" } as const;
      // A fast path, not the decision — the lock below reaches the same answer
      // on its own, and is the one that holds under a race.
      if (current.attestedAt) return { outcome: "attested" } as const;
      ...
      // a foreign key cannot reach a filesystem — and nothing sweeps them
      // (ADR 0013). Read first so the audit event can say what went.
      ...
      const [removed] = await tx
        ...
        .where(eq(schema.evidence.id, evidenceId))
        .returning();
      // Attested between the read and the delete: the policy is the authority.
      if (!removed) return { outcome: "attested" } as const;

      await c.var.audit(tx, {
        action: "deleted",
        resourceType: "evidence",
        resourceId: evidenceId,
        before: { ...fieldsOf(locked, audited), files: files.map((file) => file.filename) },
      });
  ...
  .put("/evidence/:evidenceId/attestation", knownEvidenceId, async (c) => {
    ...
    const result = await c.var.withOrganization(async (tx) => {
      ...
      const [row] = await tx
        ...
        .where(
          sql`${schema.evidence.id} = ${evidenceId} and "evidence"."xmin"::text = ${current.version}`,
        )
        .returning();
