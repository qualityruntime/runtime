...
`nextCursor` is null on the last page, so a client pages until it is null and never has to ask whether an empty page is coming.
...
Filtering was not part of the initial implementation. A filter composes with this cleanly: it narrows the rows, the ordering key is unchanged, and the cursor still names a position. `?status=active` can be added to controls without revisiting any of this. The first filter to arrive was `?resource=` on history ([ADR 0018](0018-one-history-rather-than-one-per-record.md)), and it bore that out with one wrinkle worth carrying forward: a filtered collection is a _different_ ordering, so its name has to carry the filter or a cursor will cross between them.
