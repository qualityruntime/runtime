...
Accepted. The upload shape and the mounted-volume adapter are superseded by [ADR 0021](0021-file-bytes-in-object-storage.md), which moves the bytes to S3-compatible object storage. What this ADR decided about the `file` row, the trigger, the checksum and the asymmetry of the failure boundary stands; how the boundary is judged is superseded by ADR 0021, which stopped reading the outcome out of the error.
