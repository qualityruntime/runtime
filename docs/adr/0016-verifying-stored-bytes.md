...
It also cannot, by itself, distinguish a volume whose files are gone from a volume that is not there: an unmounted or mistyped `STORAGE_DIRECTORY` answers "no bytes" for every key. So the command refuses to start when the directory is absent, and the report says so when every file it checked was missing, rather than sending someone to the backups over a typo.

There is still no sweep for the other direction: bytes on the volume that no row claims. That needs the store to list its keys, which the interface deliberately does not do, and an orphan wastes space rather than breaking trust. The test that counts paths against rows would notice one in development, which is not the same as a deployment noticing.
