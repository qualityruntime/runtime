...
Files attach to evidence and are described by `file`: the uploader-supplied filename and content type, plus the byte count and SHA-256 computed while writing. The stored size does not rely on the upload's `Content-Length` header. The bytes live in durable storage keyed by the row's identifier ([ADR 0013](adr/0013-durable-storage.md)) — PostgreSQL stays the authority on what exists and who may read it, and a key is not a permission. An attachment is kept for as long as its evidence is; a URL in the evidence's description keeps nothing and lasts only as long as whatever it points at, such as a CI artefact with a cleanup policy.
